...
describe('MagicString', () => {
  ...
  describe('generateMap', () => {
    it('should generate a sourcemap', () => {
      ...
      const mapObject = map.toMap()

      assert.equal(mapObject.version, 3)
      assert.equal(mapObject.file, 'output.md')
      assert.deepEqual(mapObject.sources, ['input.md'])
      assert.deepEqual(mapObject.sourcesContent, ['abcdefghijkl'])
      assert.equal(mapObject.mappings, 'AAAA,CAAC,CAAC,CAAO,CAAC,CAAC')

      assert.equal(
        map.toString(),
        '{"version":3,"mappings":"AAAA,CAAC,CAAC,CAAO,CAAC,CAAC","names":[],"sources":["input.md"],"sourcesContent":["abcdefghijkl"],"file":"output.md"}',
      )
      assert.equal(
        map.toUrl(),
        'data:application/json;charset=utf-8;base64,eyJ2ZXJzaW9uIjozLCJtYXBwaW5ncyI6IkFBQUEsQ0FBQyxDQUFDLENBQU8sQ0FBQyxDQUFDIiwibmFtZXMiOltdLCJzb3VyY2VzIjpbImlucHV0Lm1kIl0sInNvdXJjZXNDb250ZW50IjpbImFiY2RlZmdoaWprbCJdLCJmaWxlIjoib3V0cHV0Lm1kIn0=',
      )

      const smc = new SourceMapConsumer(mapObject)
      let loc
    ...
    it('should yield consistent results between appendLeft and prependRight', () => {
      ...
      const m1 = s1.generateMap({
        file: 'output',
        source: 'input',
        includeContent: true,
      }).toMap()
      const m2 = s2.generateMap({
        file: 'output',
        source: 'input',
        includeContent: true,
      }).toMap()
...
describe("GenerateMap", () => {
  it("should correctly map inserted multi-lines content", () =>  {
  ...
  it("should correctly map to JSON", () =>  {
    const s = new MagicString("function Foo () {}")
...
describe('overwrite', () => {
  ...
  it('should throw an error if overlapping replacements are attempted', () => {
    ...
    assert.throws(() => s.overwrite(8, 12, 'yy'))
  ...
  it('should disallow overwriting zero-length ranges', () => {
    const s = new MagicString('x')
    assert.throws(() => s.overwrite(0, 0, 'anything'))
  })

  it('should throw when given non-string content', () => {
  const s = new MagicString('')
  // @ts-expect-error
  assert.throws(() => s.overwrite(0, 1, []))
  })
...
describe('trim', () => {
  it('should trim original content', () => {
    assert.equal(
      new MagicString('   abcdefghijkl   ').trim().toString(),
      'abcdefghijkl',
    )
    assert.equal(
      new MagicString('   abcdefghijkl').trim().toString(),
...
// Original length implementation does not count `intro / outro`
// describe('length', () => {
//   it('should support length', () => {
//     const str = ' abcde   fghijkl '
